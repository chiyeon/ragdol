...
struct ASTNode
{
   ...
   virtual std::string to_str() { return token.lexeme; }
...
struct Literal : public ASTNode
{
   ...
   Literal(Token t, std::shared_ptr<Value> value) : ASTNode(t), value(value) {}
...
struct StatementList : public ASTNode
{
   ...
   StatementList(Token t, std::vector<ASTNode*>&& statements) : ASTNode(t), statements(std::move(statements))
   {
   }
...
struct Block : public ASTNode
{
   ...
   std::string to_str() override { return "Block {\n" + indent(statements->to_str() + "\n}"); }
};
...
struct Variable : public ASTNode
{
   ...
   Variable(Token t) : ASTNode(t), var_name(t.lexeme) {}

   std::shared_ptr<Value> accept(Interpreter& visitor) override;
   std::string get_var_name() { return var_name; }

   std::string to_str() { return "Variable<" + var_name + ">"; }
};
...
struct NoOp : public ASTNode
{
   ...
   std::string to_str() override { return "NoOp"; }
};
...
struct FunctionCall : public ASTNode
{
   ...
   std::string to_str() override { return "FunctionCall<" + name + ">"; }
...
struct ReturnStatement : public ASTNode
{
   ...
   ReturnStatement(Token t, ASTNode* value) : ASTNode(t), value(value) {}
   ReturnStatement(Token t) : ASTNode(t), value(nullptr) {}

   ~ReturnStatement() { delete value; }

   std::string to_str() override { return "Return: " + (value != nullptr ? value->to_str() : "Nothing"); }
...
struct WhileLoop : public ASTNode
{
   ...
   WhileLoop(Token t, ASTNode* condition, ASTNode* body) : ASTNode(t), condition(condition), body(body) {}
   ...
   std::string to_str() override { return "While: (" + condition->to_str() + "): " + body->to_str() + ";"; }

   std::shared_ptr<Value> accept(Interpreter& visitor) override;
};

template <typename ret> struct ASTVisitor
{
   /*
