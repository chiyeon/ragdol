...
typedef std::variant<std::monostate, int, bool, float, std::string, FunctionDecl*,
...
class Value
{
 public:
   ...
   static std::shared_ptr<Value> make(Type type, Data data) { return std::make_shared<Value>(type, data); }

   static std::shared_ptr<Value> make(Type type) { return std::make_shared<Value>(type); }
 ...
 public:
   Value(Type type, Data data) : type(type), data(data) {}
