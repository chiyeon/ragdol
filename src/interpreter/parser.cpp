...
ASTNode* Parser::factor()
{
   ...
   {
   default:
      std::cout << "ERROR: Found type " << t.type_to_str.at(t.type) << " and didn't know what to do"
                << std::endl;
...
{
   std::cout << "ERROR at line " << peek().line << " column " << peek().column
             << ": Expected next token to be of type " << Token::type_to_str.at(expected_type) << " but was "
             << Token::type_to_str.at(peek().type) << std::endl;
   exit(1);
}
...
ASTNode* Parser::comparison()
{
   ...
   while (peek().type == TokenType::EQ || peek().type == TokenType::NOT_EQ || peek().type == TokenType::GT ||
          peek().type == TokenType::GTE || peek().type == TokenType::LT || peek().type == TokenType::LTE)
   {
      Token t = peek();
      eat(t.type);
...
FunctionDecl* Parser::function_decl()
{
   ...
   while (parsing_args)
   {
      ...
      {
      default:
         std::cout << "ERROR: Encountered unexpected token when parsing function arguments: "
                   << Token::type_to_str.at(next.type) << std::endl;
         parsing_args = false;
         break;
      ...
      case TokenType::IDENTIFIER:
         ...
         if (encountered_params.contains(next.lexeme))
         {
            ...
            parsing_args = false;
            std::cout << "ERROR: Encountered duplicate parameter name when parsing function arguements"
                      << std::endl;
         }
         else
         {
